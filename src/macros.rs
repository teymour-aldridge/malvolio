...
*/
//! Important: these are not intended for general consumption (only use them
//! internally).
//!
//! A set of macros which are used to reduce the number of times one has to type
//! out the same thing over and over again, which I assure you is very boring
//! (repeated typing of the same thing over and over again tends to lead to
//! asking existential questions as a way to pass the time – I'm rambling here,
//! aren't I :)
...
macro_rules! impl_of_heading_new_fn {
    ($name:ident, $lowercase_name:ident) => {
        /// Create a new instance of the tag in question. Equivalent to `<tag
        /// name>::new(<text>)`, but easier to type (and therefore hopefully more
        /// ergonomic.)
        ...
        impl $name {
            ...
            /// into a `Cow<'static, str>` (for example a `&str` or a `String`).
            ...
            /// Create a new item of this type **without first sanitizing the text**.
            /// You only want this if you are certain that the text in question is
            /// safe (i.e. will not execute malicious Javascript when run.)
...
macro_rules! define_raw_attribute_fn {
    () => {
        ...
        ///
        /// Note that if you can, you should use the `attribute` method, because it
